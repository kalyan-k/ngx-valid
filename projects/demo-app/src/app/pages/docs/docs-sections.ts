...
export const DOC_SECTIONS: DocSection[] = [
  {
    id: 'overview',
    title: 'Overview',
    summary: 'ngx-valid is a policy-based validation library for Angular template-driven forms. Validation rules live in reusable policy classes; the ngxValidator directive wires controls to those policies and display strategies render errors for any UI framework.',
    bullets: [
...
validationProvider.registerFormGroupPolicy('mainForm', 'SampleForm');`
  ...
  {
    id: 'directive',
    title: 'ngxValidator Directive',
    summary: 'Attach ngxValidator to native controls or fieldsets. The directive registers the control with its form group, runs field validation on blur/change, and delegates error rendering to the active display strategy.',
    code: `<input
  [(ngModel)]="model.email"
  ngxValidator
...
/>`
  },
  {
    ...
    summary: 'groupName on ngxValidator ties fields to a form group badge (ngx-validation-group-status). Policy groups aggregate multiple sections for page-level status and summaries.',
    bullets: [
  ...
  },
  {
    ...
    bullets: [
      'ngx-validation-group-status — Valid / Invalid / Not validated badge',
      'ngx-validation-group-summary — errors for one form group',
      'ngx-validation-policy-group-status — combined badge across sections',
      'ngx-validation-summary — all errors on the model after showAllErrors'
    ]
  },
  {
    id: 'api',
